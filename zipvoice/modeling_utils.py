...
def process_audio(audio, transcriber, tokenizer, feature_extractor, device, target_rms=0.1, duration=4, feat_scale=0.1):
    ...
    # 🚨 关键修改：显式传 array + sampling_rate，绕开 torchcodec
    prompt_text = transcriber(
        {"array": prompt_wav2, "sampling_rate": sr}
    )["text"]
    print(prompt_text)
...
def load_models_gpu(model_path=None, device="cuda"):
    ...
    transcriber = pipeline("automatic-speech-recognition", model="whisper-base/snapshots/e37978b90ca9030d5170a5c07aadb050351a65bb", device=device)
